...
public class Looogs implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < l.length; i++) {
            l[i] = new Thread(new Looogs()); //nowy watek, ktory jako parametr przyjmuje LogsFight
            l[i].start();
        }
    ...
    public void run() {
        try {
            BufferedWriter pw = new BufferedWriter(new FileWriter("3Logs.txt", true));
            ...
            BufferedReader br = new BufferedReader(new FileReader("3Logs.txt"));
